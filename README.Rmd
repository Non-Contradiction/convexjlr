...
- `XRJulia` connects to `julia`, which is the default way for `convexjlr`,
  ...
  latest version of `XRJulia` on github:
...
- `JuliaCall` embeds `julia` in R,
  ...
  compilations, on some types of machines the installation process may be more
  complicated than `XRJulia`.

And there are several backward incompatible issues in `julia` v0.6 and `julia` v0.5 and corresponding
version of `Convex.jl`,
so if you are using `julia` v0.6, please use `convexjlr` with care.
